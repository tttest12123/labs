...
from lab_2.show_code import CODE
...
def show():
    ...
    if st.button("Train Network"):
        def target_func(x, y, func_str):
            allowed_names = {k: v for k, v in np.__dict__.items() if callable(v)}
            allowed_names.update({'x': x, 'y': y})
            return eval(func_str, {"__builtins__": None}, allowed_names)

        X = np.random.uniform(-5, 5, (1000, 2))
        y = target_func(X[:, 0], X[:, 1], func_str)
